...
# Окно выбора типа письма.
    def open_letters_window(self):
        letters_window = tk.Toplevel(self)
        letters_window.title("Выберите тип письма")
        ...
        # Получаем размеры экрана
        screen_width = letters_window.winfo_screenwidth()
        screen_height = letters_window.winfo_screenheight()
        ...
        letters_window.geometry(f"{window_width}x{window_height}+{x}+{y}")

        # Кнопки
        simple_button = tk.Button(letters_window, text="   Простые письма   ", command=self.calculate_simple_letters)
        simple_button.pack(pady=(20, 0))  # Устанавливаем отступ сверху для первой кнопки

        tk.Label(letters_window, text="").pack()

        registered_button = tk.Button(letters_window, text=" Заказные письма ", command=self.calculate_registered_letters)
        registered_button.pack(pady=(10, 0),)  # Устанавливаем отступ снизу для второй кнопки

        tk.Label(letters_window, text="").pack()

        foreign_button = tk.Button(letters_window, text=" Иностранные ", command=self.calculate_foreign_letters)
        foreign_button.pack(pady=(10, 0),)  # Устанавливаем отступ снизу для второй кнопки

        # Надпись "by.Borzzz" в нижнем правом углу
        by_label = tk.Label(letters_window, text="by.Borzzz", fg="gray")
        by_label.pack(side=tk.BOTTOM, anchor=tk.SE, padx=10, pady=10)

        letters_window.focus_set()
...
# Отыкрывается кнопка ПРОСТЫХ писем  
    def calculate_simple_letters(self):
        ...
        # Кнопка "удалить выбранное" для удаления конкретного введенного результата
        self.delete_selected_button = tk.Button(self.simple_window, text="Удалить выбранное", command=self.remove_selected)
        self.delete_selected_button.pack(pady=(5,10))

        # Кнопка "Закончить подсчет" для открытия календаря
        self.finish_button = tk.Button(self.simple_window, text="Закончить подсчет", command=self.open_calendar)
        self.finish_button.pack(pady=(5, 10))
