...
# Это главное окно с последующими кнопками
    def create_widgets(self):
        ...
        self.doc_button = tk.Button(
            self, text="Создать обложку", command=self.open_calendar_and_create_document
        )
...
# Это сохранение итогов (за определенный месяц)
    def calculate_total_for_month(self, event=None):
        try:
            ...
            selected_month = re.sub(r'[\s\\\/., ]', '.', selected_month)
...
# Кнопка создания облложки
    def open_calendar_and_create_document(self):
        # Создаем окно
